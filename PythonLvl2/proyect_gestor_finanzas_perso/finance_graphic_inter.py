...
def open_add_expense_window(gestor, data_manager):
    ...
    while True:
        ...
        if event_ex == sg.WINDOW_CLOSED or event_ex == "Cancel":
            break
...
def add_income_category(gestor, data_manager):
    ...
    while True:
        ...
        try:
            ...
            data_manager.save_categories(gestor.available_income_features, r"data/income_categories.json" )
...
def add_expense_category(gestor, data_manager):
    ...
    while True:
        ...
        try:
            ...
            data_manager.save_categories(gestor.available_spent_categories, r"data/expense_categories.json" )
            sg.popup("Category added to the list!")
            break
        except Exception as e: 
            sg.popup_error (str(e))
...
def buid_movements_table(gestor):
    data = []
    for mov in gestor.obtain_all_movements():
        data.append([mov.title,
                    mov.amount,
                    mov.category,
                    mov.date,
...
def filter_movements_by_date(gestor, start_str, end_str):
    start = datetime.strptime(start_str, "%d/%m/%Y")
    end = datetime.strptime(end_str, "%d/%m/%Y")

    filtered = []
    for mov in gestor.obtain_all_movements():
        mov_date = datetime.strptime(mov.date, "%d/%m/%Y")
...
main_layout = [[sg.Text("Welcome to your FinancialAdmin!")],
            ...
            [sg.Text("From (dd/mm/yyyy):"), sg.Input(key="-START_DATE-", size=(12, 1)),
            sg.Text("To (dd/mm/yyyy):"), sg.Input(key="-END_DATE-", size=(12, 1)),
            sg.Button('FilterDate')],
...
loaded_spent_categories = data_manager.load_categories(r"data/expense_categories.json")
if loaded_spent_categories:
    gestor.available_spent_categories = loaded_spent_categories

loaded_income_categories = data_manager.load_categories(r"data/income_categories.json")
if loaded_income_categories:
    gestor.available_income_features = loaded_income_categories
...
while True:
    ...
    if event == "Add Expense":
        ...
        window["-MOVEMENTS-TABLE-"].update(values=buid_movements_table(gestor))
    if event == "Add Income":
    ...
    if event == "FilterDate":
        ...
        data = filter_movements_by_date(gestor, start_str, end_str)
        window["-MOVEMENTS-TABLE-"].update(values=data)
